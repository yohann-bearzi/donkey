#!/usr/bin/env python3
"""donkey_train v2 - latent world model over MiMo-V2.5 hiddens, token-anchored (MLX)."""
...
STOP = (151645, 151643)
DMODEL = 4096; SEED = 0
SIGREG_LAMBDA = 0.1; SIGREG_M = 1024
...
def build_transitions(h, tok, pid, W, keep_only_terminated=True):
    ...
    return (np.asarray(win,np.int64), np.asarray(act,np.int32),
            np.asarray(tgt,np.int64), np.asarray(src,np.int64))
...
def make_modules(mx, nn, d, dp, n_enc, n_dec, n_actions):
    class Block(nn.Module):
        ...
        def __call__(self,x,a_emb=None,mask=None):
            ...
            o=(att@v).transpose(0,2,1,3).reshape(B,T,W); x=x+self.o(o)
...
def load_into(mx,module,path):
    from mlx.utils import tree_unflatten
    module.update(tree_unflatten(list(mx.load(path).items()))); return json.load(open(path+".meta.json"))
...
def stage_ae(args, mx, nn, optim):
    mx.random.seed(SEED); rng=np.random.default_rng(SEED)
    h,tok,pid = load_trace(args.corpus)
    wi,act,tgt,src = build_transitions(h,tok,pid,WINDOW)
    pos = np.unique(np.concatenate([src,tgt]))
    n_actions = int(np.unique(act).size)
    phi,pred,psi,aemb = make_modules(mx,nn,args.d,args.dp,args.enc,DEC_LAYERS,max(n_actions,1))
    sigreg = make_sigreg(mx)
    mu,sd = _NORM[args.corpus]; project = load_lm_head(mx)(mu,sd)
    class AE(nn.Module):
        def __init__(s): super().__init__(); s.phi=phi; s.psi=psi
    ae=AE()
    print(f"[stage AE] {args.corpus} d={args.d} dp={args.dp} | {nparams(mx,ae)/1e6:.1f}M params")
    upid=np.unique(pid); rng.shuffle(upid); nval=max(1,int(0.1*len(upid)))
    val_pids=set(upid[:nval].tolist())
    val_mask=np.array([p in val_pids for p in pid[pos]])
    tr_pos=pos[~val_mask]; va_pos=pos[val_mask]
    def loss(hb):
        z=ae.phi(hb[:,None,:])[:,0,:]; hrec=ae.psi(z)
        ce=mx.mean(nn.losses.cross_entropy(project(hrec), mx.argmax(project(hb),axis=-1)))
        return ce + SIGREG_LAMBDA*sigreg(z)
    lg=nn.value_and_grad(ae,loss); opt=optim.AdamW(learning_rate=args.lr)
    best=None; bad=0; bs=args.bs
    for ep in range(args.epochs):
        t0=time.time(); rng.shuffle(tr_pos); tot=nb=0
        for i in range(0,len(tr_pos)-bs+1,bs):
            hb=mx.array(h[tr_pos[i:i+bs]]); L,g=lg(hb)
            g=clip_grads(mx,g,1.0); opt.update(ae,g); mx.eval(ae.parameters(),opt.state); tot+=float(L); nb+=1
        vb=va_pos[rng.choice(len(va_pos),min(4096,len(va_pos)),replace=False)]
        hb=mx.array(h[vb]); z=ae.phi(hb[:,None,:])[:,0,:]; hrec=ae.psi(z)
        ceil=float((np.array(mx.argmax(project(hrec),axis=-1))==np.array(mx.argmax(project(hb),axis=-1))).mean())
        imp = best is None or ceil>best
        if imp: best=ceil; bad=0; save_ckpt(mx,args.out,ae,{"d":args.d,"dp":args.dp,"enc":args.enc,"n_actions":n_actions,"corpus":args.corpus})
        else: bad+=1
        print(f"[ae ep{ep+1}/{args.epochs}] loss {tot/nb:.4f} | val recon-ceiling {ceil*100:.1f}%{' *' if imp else ''} | bad {bad} | {time.time()-t0:.1f}s")
        if bad>=args.patience: print(f"[ae] early stop (patience {args.patience})"); break
    print(f"[ae] best val ceiling {best*100:.1f}% -> {args.out}")

def stage_pred(args, mx, nn, optim):
    mx.random.seed(SEED); rng=np.random.default_rng(SEED)
    aem=json.load(open(args.ae+".meta.json")); d,dp,enc=aem["d"],aem["dp"],aem["enc"]
    ...
    phi,pred,psi,aemb = make_modules(mx,nn,d,dp,enc,DEC_LAYERS,len(uniq))
    class AE(nn.Module):
        def __init__(s): super().__init__(); s.phi=phi; s.psi=psi
    load_into(mx,AE(),args.ae)
    mask=causal_mask(mx,WINDOW)
    mu,sd=_NORM[args.corpus]; project=load_lm_head(mx)(mu,sd)
    class PRED(nn.Module):
        def __init__(s): super().__init__(); s.pred=pred; s.aemb=aemb
    pm=PRED()
    print(f"[stage PRED] {args.corpus} d={d} dp={dp} | {nparams(mx,pm)/1e6:.1f}M params (phi+psi frozen)")
    ...
    if val_corpus!=args.corpus:
        ...
        aixv=np.array([remap.get(int(t),0) for t in actv],np.int32)
        projv=load_lm_head(mx)(*_NORM[val_corpus])
    else:
    ...
    def fcos(zhat,zt):
        zhn=zhat/(mx.linalg.norm(zhat,axis=-1,keepdims=True)+1e-6)
        ztn=zt/(mx.linalg.norm(zt,axis=-1,keepdims=True)+1e-6)
        return 1-mx.sum(zhn*ztn,axis=-1)
    def loss(hw,a,ht,w):
        z=phi(hw)
        zt=mx.stop_gradient(phi(ht[:,None,:])[:,0,:])
        zhat=pm.pred(z,pm.aemb(a),mask)
        Lcos=mx.sum(w*fcos(zhat,zt))/(mx.sum(w)+1e-6)
        hrec=psi(zhat)
        tgt_tok=mx.argmax(project(mx.stop_gradient(ht)),axis=-1)
        Lce=mx.mean(nn.losses.cross_entropy(project(hrec),tgt_tok))
        return Lcos + CE_LAMBDA*Lce
    lg=nn.value_and_grad(pm,loss); opt=optim.AdamW(learning_rate=args.lr)
    def val_accept():
        k=min(4096,wiv.shape[0]); b=rng.choice(wiv.shape[0],k,replace=False)
        hw=mx.array(hv[wiv[b]]); a=mx.array(aixv[b]); ht=mx.array(hv[tgtv[b]])
        zhat=pm.pred(phi(hw),pm.aemb(a),mask); e2e=np.array(mx.argmax(projv(psi(zhat)),axis=-1))
        trunk=np.array(mx.argmax(projv(ht),axis=-1)); return float((e2e==trunk).mean())
    ...
    for ep in range(args.epochs):
        ...
        for i in range(0,T-bs+1,bs):
            ...
            L,g=lg(hw,a,ht,w); g=clip_grads(mx,g,1.0); opt.update(pm,g); mx.eval(pm.parameters(),opt.state); tot+=float(L);nb+=1
        acc=val_accept(); imp=best is None or acc>best
        if imp: best=acc;bad=0; save_ckpt(mx,args.out,pm,{"d":d,"dp":dp,"enc":enc,"ae":args.ae,"val":val_corpus})
        else: bad+=1
        print(f"[pred ep{ep+1}/{args.epochs}] loss {tot/nb:.4f} | val({val_corpus}) ACCEPT {acc*100:.2f}%{' *' if imp else ''} | bad {bad} | {time.time()-t0:.1f}s")
        if bad>=args.patience: print(f"[pred] early stop"); break
    print(f"[pred] best val accept {best*100:.2f}% -> {args.out}")

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("corpus",choices=sorted(CORPORA))
    ap.add_argument("--stage",choices=["ae","pred"],required=True)
    ...
    ap.add_argument("--ae",type=str,default=None); ap.add_argument("--val",type=str,default=None)
    ap.add_argument("--epochs",type=int,default=60); ap.add_argument("--bs",type=int,default=256)
    ap.add_argument("--lr",type=float,default=1e-4); ap.add_argument("--patience",type=int,default=8)
    ap.add_argument("--out",type=str,default=None)
    a=ap.parse_args()
    os.makedirs(CKPTS,exist_ok=True)
    if a.out is None: a.out=os.path.join(CKPTS, f"{a.stage}_{a.corpus}_d{a.d}_dp{a.dp}.npz")
    import mlx.core as mx, mlx.nn as nn, mlx.optimizers as optim
    if a.stage=="ae": stage_ae(a,mx,nn,optim)
    else:
        assert a.ae, "--stage pred needs --ae <autoencoder.npz>"
        stage_pred(a,mx,nn,optim)
