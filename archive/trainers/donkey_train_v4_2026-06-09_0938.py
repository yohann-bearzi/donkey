...
def train(args):
    ...
    def loss_fn(model, zwin, action, depth, h_tgt_std, p_ids, p_probs):
        ...
        L = t_cos + t_dist + t_rec + t_sig + t_l2 + t_shape
        return L, (mx.mean(cos), mx.mean(jsd_pred), mx.mean(gate),
                   t_cos, t_dist, t_rec, t_sig, t_l2, t_shape, dcos)
    ...
    for ep in range(start_ep, args.epochs + 1):
        ...
        cos_m, jsd_m, gate_m, Lcos, Ldist, Lrec, Lsig, Ll2, Lshape, dcos_m = aux
        ...
        print(f"[v4 ep{ep}/{args.epochs}] loss {tot_loss/nb:.4f} | "
              ...
              f"L[cos {float(Lcos):.3f} dist {float(Ldist):.3f} rec {float(Lrec):.3f} sig {float(Lsig):.3f} l2 {float(Ll2):.3f} shape {float(Lshape):.3f}] | "
